...
PATH_TO_DRIVER = os.path.expanduser("~") + "/nrc/nrc.ko"
PATH_TO_CONFIG = os.path.expanduser("~") + "/{}-interface"
...
class Recovery:
	...
	def startup(self): #example
		...
		time.sleep(5)
		os.system("sudo {} {} {}/{} -B > /dev/null".format(app, "-iwlan0 -c" if self.is_sta() else "", PATH_TO_CONFIG.format(station), self.get_config()))
		time.sleep(7)
